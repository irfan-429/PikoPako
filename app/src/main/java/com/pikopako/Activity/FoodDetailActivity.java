                    ...
                    try {
                        ...
                        if (jsonObject1.getString("status").equalsIgnoreCase(Constant.SUCCESS)) {
                            ...
                            if (toppingarray.length() > 0)
                                BaseApplication.getInstance().getSession().setIsToppingAvble(true);
                            else
                                BaseApplication.getInstance().getSession().setIsToppingAvble(false);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == 225) {
                ...
                onActResult(data, true);
            }
        }
    }

    private void onActResult(Intent data, boolean toppingAvailable) {
        clearListOfList = false;
//                Log.e(TAG, "onActivityResult: checking ");

//                Log.e(TAG, "onActivityResult 225: " + data.getParcelableArrayListExtra("data").toString());

        ArrayList<Ingrediants_modal> newToppingsList;
        if (toppingAvailable)
            newToppingsList = (ArrayList<Ingrediants_modal>) data.getSerializableExtra("data");
        else newToppingsList = new ArrayList<>();
...
//                Log.e("===>>", "getParcelableArrayListExtra: " + data.getParcelableArrayListExtra("data").toString());
        Log.e(TAG, "SIZE: checking " + newToppingsList.size());

        //    addProduct(newToppingsList1, product_id, customize, pos);
        add_layout.setVisibility(View.GONE);
        add_quantity_layout.setVisibility(View.VISIBLE);
        snackbar_layout.setVisibility(View.VISIBLE);
        toppings_price = 0;
        ...
        //  List_of_list.add(parse_into_new_list(newToppingsList1));
        Log.e("===>>", "List_of_list.size(): " + List_of_list.size());
        ...
        for (int i = 0; i < newToppingsList.size(); i++) {
            ...
            Log.e(TAG, "toppings_price1 " + toppings_price);
